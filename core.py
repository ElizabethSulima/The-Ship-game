...
from PyQt5.QtCore import (  # pylint:disable=E0611
    QPoint,
    QPropertyAnimation,
    QRect,
    QSize,
...
from PyQt5.QtGui import QPalette, QPixmap  # pylint:disable=E0611
...
class Image(QLabel):
    ...
    def setPosition(self, x: int, y: int) -> None:
        self.setGeometry(
            BORDER_SIZE + CELL_SIZE * int(x),
            BORDER_SIZE + CELL_SIZE * int(y),
            CELL_SIZE,
            CELL_SIZE,
        )
...
class Marker(Image):
    ...
    def moveTo(self, x: int, y: int) -> None:
        self.anim.setEndValue(
            QPoint(
                BORDER_SIZE + int(x) * CELL_SIZE,
                BORDER_SIZE + int(y) * CELL_SIZE,
            )
        )
    ...
    def setHealth(self, part: float) -> None:
        self.healthBar.setGeometry(
            5, CELL_SIZE - 5, int((CELL_SIZE - 10) * part), 5
        )
...
class GameAPI(QMainWindow):
    def __init__(self, game):
        ...
        self.log.setGeometry(
            CELL_SIZE * GRID_SIZE + BORDER_SIZE * 2,
            0,
            400,
            CELL_SIZE * GRID_SIZE + BORDER_SIZE * 2,
        )
    ...
    def mousePressEvent(self, event):
        ...
        if 0 <= x < GRID_SIZE and 0 <= y < GRID_SIZE:
            try:
                ...
                print(
                    "Exception while handling click at %s, %s: %s"
                    % (event.x(), event.y(), e)
                )
                traceback.print_exc()
    ...
    def addImage(self, path: str, x: int, y: int) -> Image:
        assert isinstance(path, str), (
            "Wrong argument path on GameAPI.removeSprite(): %s" % path
        )
        img = Image(path)
        img.setParent(self)
        img.setPosition(x, y)
